...
function getConfigById(configId) {
    ...
    return state.configs.find(c => c.id === configId) || null;
}
...
async function saveThreadSettings() {
    ...
    if (r1.success && r2.success && r3.success) {
        ...
        console.log('[线程设置] 已保存', { maxConcurrent, threadPoolSize, delayMin, delayMax });
    } else {
...
async function refreshConfigs() {
    ...
    if (data.success) {
        ...
        console.log('[配置列表] 已刷新', state.configs);
        renderConfigList();
        renderBatchFiles();
...
async function saveConfig() {
    ...
    console.log('[保存配置] payload=', payload);
...
async function handleBatchFiles(files) {
    ...
    console.log('[批处理文件] 当前state.batchFiles =', JSON.parse(JSON.stringify(state.batchFiles)));
...
function renderBatchFiles() {
    ...
    container.innerHTML = state.batchFiles.map(f => {
        ...
        const configName = getConfigNameById(f.configId);
        ...
        <div class="batch-file-card" data-file-id="${f.fileId}">
            ...
            <div style="margin:6px 0 10px 0;font-size:12px;color:var(--text-muted);">
                当前配置名称：${escapeHtml(configName)}
                <br>
                当前配置ID：<code>${escapeHtml(f.configId || '__default__')}</code>
            </div>
...
function updateBatchFileConfig(fileId, configId) {
    ...
    f.configId = configId || '';
    console.log('[切换批处理文件配置]', {
        fileId,
        fileName: f.fileName,
        newConfigId: f.configId,
        newConfigName: getConfigNameById(f.configId)
    });
...
async function submitBatch() {
    ...
    console.log('[提交前] state.batchFiles =', JSON.parse(JSON.stringify(state.batchFiles)));
    ...
    console.log('[提交批处理] files payload =', JSON.parse(JSON.stringify(files)));
        ...
        ${(task.files || []).map((f, idx) => `
            <div class="card" style="margin-bottom:12px;">
                ...
                <div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px;">
                    ...
                    · configId：${escapeHtml(f.config_id || '__default__')}
